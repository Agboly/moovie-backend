# Importer tous les modules necessaires au bon fonctionnement de l'API
...
app = FastAPI( 
    title="MovieLens API",  
    description="Bienvenue dans l'API **MovieLens** Cette API permet d'interagir avec une base de données inspirée du célèbre jeu de données [MovieLens](https://grouplens.org/datasets/movielens/). Elle est idéale pour découvrir comment consommer une API REST avec des données de films, d'utilisateurs, d'évaluations, de tags et de liens externes (IMDB, TMDB). ",  
    version="0.1" 
) 

# Definir une fonction de dépendance pour récupérer une session de base de données en ouverture et fermeture de la session pour chaque requette
def get_db(): 
    db = SessionLocal()
    try: 
        yield db 
...
# Il faut marquer les différents points de terminaison...Le 1er etant Endpoint de health check pour tester la santé de l'API
@app.get( 
    "/", 
    summary="Vérifie si l'API MovieLens fonctionne", 
    description="Ce point d'entrée permet de vérifier si l'API MovieLens est opérationnelle.", 
...
# Endpoint pour obtenir une liste des lms (avec pagination et ltres facultatifs title, genre, skip, limit)
@app.get( 
    "/movies", 
